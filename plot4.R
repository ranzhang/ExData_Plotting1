...
##generate plots
par(mfrow = c(2,2), mar=c(4,2,2,0)) ##2x2 plots

#plot1
plot(tmp$DateTime, tmp$Global_active_power, type = "l", xlab="", ylab="Global Active Power (kilowatts)")

#plot2
plot(tmp$DateTime, tmp$Voltage, type = "l", xlab="datetime", ylab="Voltage")
...
legend("top", lwd=1, lty=1, col = c("black", "red", "purple"), bty="n", cex = 0.7, legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))

#plot4
plot(tmp$DateTime, tmp$Global_reactive_power, xlab="datetime", ylab="Global_reative_power", type = "l")
